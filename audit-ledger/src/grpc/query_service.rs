...
pub struct AuditQueryServer<T: QueryServiceRpc> {
    inner: Arc<T>,
}

impl<T: QueryServiceRpc> AuditQueryServer<T> {
...
impl<T: QueryServiceRpc> Clone for AuditQueryServer<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: QueryServiceRpc> tonic::codegen::Service<http::Request<tonic::body::BoxBody>>
    for AuditQueryServer<T>
{
...
impl<T: QueryServiceRpc> tonic::server::NamedService for AuditQueryServer<T> {
    const NAME: &'static str = "sentinel.audit.QueryService";
}
